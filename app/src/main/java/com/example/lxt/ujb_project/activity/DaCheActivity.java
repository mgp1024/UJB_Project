...
public class DaCheActivity extends Activity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// 创建缓存
		try {
			File path = getDiskCacheDir(DaCheActivity.this, "File");
			if (!path.exists()) {
				path.mkdirs();
			}
			mDiskLruCache = DiskLruCache.open(path, getAppVersion(DaCheActivity.this), 1, 10 * 1024 * 1024);
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	protected void onResume() {
		...
		myListView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				...
				if (size != 0) {
					Intent intent = new Intent(DaCheActivity.this, DriverItemActivity.class);
	...
	private Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case Net_OK:
				...
				driverAdapter = new DriverAdapter(DaCheActivity.this, 0, drivers, myListView);
	...
	public ArrayList<Driver> getLocalNews() {
		String url = URLCollect.BASE_URL + URLCollect.DRIVER_URL + "page="+page+"&cate=0&pagesize=10";
	...
	public void  getSimulationNews() {
		...
		final String url = URLCollect.BASE_URL + URLCollect.DRIVER_URL + "page="+page+"&cate=0&pagesize=10";
	...
	class LvOnRefreshListener implements OnRefreshListener2<ListView> {
		...
		public void onPullDownToRefresh(PullToRefreshBase<ListView> refreshView) {
			...
			String label = DateUtils.formatDateTime(DaCheActivity.this.getApplicationContext(), System.currentTimeMillis(), DateUtils.FORMAT_SHOW_TIME | DateUtils.FORMAT_SHOW_DATE | DateUtils.FORMAT_ABBREV_ALL);
	...
	class GetUpTask extends AsyncTask<String, Void, Integer> {
		...
		protected void onPostExecute(Integer result) {
			super.onPostExecute(result);
			page = page + 1;
			if (NetUtils.isConnected(DaCheActivity.this) == true) {
				getSimulationNews();
			} else {
				getLocalNews();
			}
